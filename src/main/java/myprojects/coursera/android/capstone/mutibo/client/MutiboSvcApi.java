package myprojects.coursera.android.capstone.mutibo.client;
...
/*
 * This interface defines an API for a MutiboSvc. The
 ...
 *      using Spring Data. The list of QuizSet objects should be able 
 *      to be unmarshalled by the client into a Collection<QuizSet>.
 ...
 *      QuizSet class when deserialized by Spring's default 
 *      Jackson library.
 *    - Returns the JSON representation of the QuizSet object that
 *      was stored along with any updates to that object made by the server. 
 *    - **_The server should store the QuizSet in a Spring Data JPA repository.
 ...
 *    - You will need to add one or more annotations to the QuizSet object
 ...
 *      prevent a user from liking a video twice. A POJO QuizSet object is provided for 
 ...
 *     
 *     
 * The MutiboSvcApi interface described below should be used as the ultimate ground
 * truth for what should be implemented in the assignment. If there are any details
 * in the description above that conflict with the MutiboSvcApi interface below, use
 * the details in the MutiboSvcApi interface and report the discrepancy on the course
 * forums. 
 * 
...
public interface MutiboSvcApi {
	...
	public static final String COMBINATION_PARAMETER = "combination";	
...
//	public static final String VIDEO_TITLE_SEARCH_PATH = MOVIESET_SVC_PATH + "/search/findByName";
	...
	// The path to search videos by title
	public static final String COMBINATION_LESSTHAN_SEARCH_PATH = MOVIESET_SVC_PATH + "/search/findByCombinationLessThan";
	...
	@GET(COMBINATION_LESSTHAN_SEARCH_PATH)
	public Collection<QuizSet> findByCombinationLessThan(@Query(COMBINATION_PARAMETER) long combination);
	
	//movie/search/findByCombinationLessThan/
